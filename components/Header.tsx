    ...
    return (
        <header className="flex flex-row z-10 px-5 md:px-20 min-[1650px]:px-48 py-3 bg-blue-500 text-white">
            ...
            <div className="flex flex-row gap-2 px-2 place-items-center justify-self-center rounded-full">
                <Link href="/" className={`text-lg leading-6 w-14 ${currentRoute == "/" || currentRoute?.includes("/search") ? 'font-bold' : ''}`}> Search</Link>
                <Link href="/favorites" prefetch={false} className={`text-lg leading-6 w-14 ${currentRoute == "/favorites" ? 'font-bold' : ''}`}> Saved</Link>
            </div>
